...
- A Specification File (spec.yml) that defines the slides to be generated.
...
Next, pass the outputs object to the `adding_ai_footnotes()` function. This function iterates through your outputs, and for each one that has a corresponding prompt, it communicates with the specified LLM to generate response in the form of a footnote.

You can configure the function to point to different AI platforms.

Example 1: Using a Local LLM (Ollama)
This is ideal for local development. It assumes you have Ollama running, for instance, in a Docker container.
...
Finally, take the modified outputs_with_footnotes object and pass it to generate_slides() to create the presentation. The footnotes will be automatically included on the relevant slides.
...
This will produce a PowerPoint file with your tables and plots, now enhanced with insightful AI-generated footnotes.
