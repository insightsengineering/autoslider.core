...
[`autoslider.core`](CRAN link) package, whereas the `autoslideR` package remaining internal, and once the template functions are stable, generic, we also encourage to make these functions open-source for external users. This article describes the how to process. Template that contains specific variable names or columns are not recommended for open-sourcing. The key steps include the following
...
1. Move `t_dm_slide.R` file to `autoslider.core` `R/` directory. 
2. Copy `tests/testthat/test-t_dm_slide.R` to `autoslider.core` `tests/testthat/`.
3. Add the following code to `R/autoslider_core.R` file.
